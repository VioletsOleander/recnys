...
def assert_synced_correctly(tasks: list[FileSyncTask]) -> None:
    for task in tasks:
        ...
        if task.policy == FileSyncPolicy.SYMLINK:
            assert file_path.is_symlink(), f"Expected {file_path} to be a symlink, but it is not."
            assert file_path.resolve() == task.src.resolve(), (
                f"Expected symlink {file_path} to point to {task.src}, "
                f"but it points to {file_path.resolve()}."
            )
        else:
            expected_content = (
                RENDERED_CONTENT
