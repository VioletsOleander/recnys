...
class TestSyncDecision:
    ...
    def test_sync_decision_values(self) -> None:
        ...
        assert (
            SyncDecision.SRC_MODIFIED.value
            == "Source File is modified (hash mismatch) since last sync"
        )
        assert (
            SyncDecision.DST_MISSING.value
            == "Destination file does not exist or is deleted since last sync"
        )
        assert (
            SyncDecision.DST_MODIFIED.value
            == "Destination file is modified (hash mismatch) since last sync"
        )
