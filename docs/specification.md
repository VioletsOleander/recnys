...
For a certain platform, if the destination path is not provided, it will be derived from the source path with a default rule:
...
> Note: All paths specified in `recnys.yaml` should be relative to the dotfiles repository root, i.e. the parent directory of `recnys.yaml`.
